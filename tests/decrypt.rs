...
fn success() {
    ...
    let decrypted = Decryptor::new(PASSWORD, TEST_DATA_ENC)
...
fn incorrect_password() {
    let decrypted = Decryptor::new("passphrase", TEST_DATA_ENC)
        .and_then(Decryptor::decrypt_to_vec)
        .unwrap_err();
    assert_eq!(
        decrypted.to_string(),
...
fn invalid_params() {
    ...
    data_1[7] = 65;
    let decrypted = Decryptor::new(PASSWORD, data_1)
        .and_then(Decryptor::decrypt_to_vec)
        .unwrap_err();
    assert_eq!(
        decrypted.to_string(),
    ...
    let decrypted = Decryptor::new(PASSWORD, data_2)
        .and_then(Decryptor::decrypt_to_vec)
        .unwrap_err();
    assert_eq!(
        decrypted.to_string(),
    ...
    let decrypted = Decryptor::new(PASSWORD, data_3)
        .and_then(Decryptor::decrypt_to_vec)
        .unwrap_err();
    assert_eq!(
        decrypted.to_string(),
